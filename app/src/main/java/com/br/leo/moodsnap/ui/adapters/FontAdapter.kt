...
class FontAdapter(
    ...
    private var selectedFontId: String? = null
    private var selectedViewHolder: FontViewHolder? = null
    ...
    inner class FontViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        private var currentFont: FontModel? = null

        init {
            // Clique direto SEM debounce para resposta instantânea
            rootLayout.setOnClickListener {
                currentFont?.let { font ->
                    selectItem(font, this)
                }
            }
        }
    ...
    private fun selectItem(font: FontModel, newViewHolder: FontViewHolder) {
        // Se já está selecionado, não fazer nada
        if (selectedFontId == font.id) return

        // Deselecionar o item anterior (se houver)
        selectedViewHolder?.setNormalVisual()

        // Selecionar o novo item
        selectedFontId = font.id
        selectedViewHolder = newViewHolder
        newViewHolder.setSelectedVisual()
    ...
    fun setSelectedFont(fontId: String) {
        ...
        selectedViewHolder = null // Reset para ser recalculado no bind
        // Encontrar e atualizar apenas o item necessário
    ...
    fun getSelectedFont(): String? = selectedFontId
}